...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      ...
      ClassName: App.globalData.userInfo.SDDetailName || "大1班" //这里添加picker中选择的ClassName
    })
    console.log("history")
  ...
  getClassList() {
    let _this = this;
    let url = "api/XXYXT/getClassList";
    let prams = null;
    App._post_form(url, prams, function (res) {
      ...
      if (result.code == 1) {
        ...
        _this.setData({
          count: result.count,
          className_array: classArr
        })
        console.log("===_this.data.className_array====", _this.data.className_array)
        _this.setData({ ClassName: _this.data.className_array[0] })
        console.log("ClassName", _this.data.ClassName)
        _this.getStudentListByClassName();
      } else {
  ...
  bindClassNamePickerChange(e){
    let _this = this;
    console.log('picker发送选择改变，携带值为', e.detail.value);
    _this.setData({
      clssName_index: e.detail.value,
      ClassName: _this.data.className_array[e.detail.value]
    });
    _this.getStudentListByClassName();
  },
  ...
  getStudentListByClassName() {
    ...
    console.log("getStudentListByClassName", prams)
    App._post_form(url, prams, function (res) {
      let result = JSON.parse(res)
      console.log("result", result)
      if (result.code == 1) {
